<?php
	class WpFastestCacheCreateCache extends WpFastestCache{
			...
			if(preg_match("/Mediapartners-Google|Google\sWireless\sTranscoder/i", $_SERVER['HTTP_USER_AGENT'])){
				...
				if($this->err){
					...
					if($this->cdn){
						...
						$content = preg_replace_callback("/srcset\=[\'\"][^\'\"]+".preg_quote($this->cdn->originurl, "/")."[^\'\"]+[\'\"]/i", array($this, 'cdn_replace_urls'), $content);
		...
		public function cdn_replace_urls($matches){
			if(preg_match("/".preg_quote($this->cdn->originurl, "/")."/i", $matches[0])){
				if(preg_match("/^srcset\=[\'\"]/", $matches[0])){
