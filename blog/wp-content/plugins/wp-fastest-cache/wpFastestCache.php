	...
	class WpFastestCache{
	    	...
	    	foreach ((array)$crons as $cron_key => $cron_value) {
	    		foreach ( (array) $cron_value as $hook => $events ) {
	    			if(preg_match("/^wp\_fastest\_cache(.*)/", $hook, $id)){
		    			...
		    			if(preg_match("/Preload/i", $id[1])){
		    				add_action("wp_fastest_cache".$id[1],  array($this, 'create_preload_cache'));
		    			}
		...
		public function set_preload(){
			...
			if($data = get_option("WpFastestCachePreLoad")){
				...
				wp_schedule_single_event(time() + 5, $this->slug()."_Preload_".(time()+5));
			}else{
				if(!empty($preload_arr)){
				...
				if(count($urls) > 0){
					foreach ($urls as $key => $arr) {
						...
						if($arr["user-agent"] == "desktop"){
							...
							$user_agent = "WP Fastest Cache Preload Mobile Bot";
						}

						$this->wpfc_remote_get($arr["url"], $user_agent);
					}

			    	wp_schedule_single_event(time() + 50, $this->slug()."_Preload_".(time()+50));
			...
			if($args){
				...
				if($rule->prefix == "all"){
					...
					if(isset($this->options->wpFastestCachePreload_homepage) && $this->options->wpFastestCachePreload_homepage){
						$this->wpfc_remote_get(get_option("home"), "WP Fastest Cache Preload Bot");
						$this->wpfc_remote_get(get_option("home"), "WP Fastest Cache Preload Mobile Bot");
					}
